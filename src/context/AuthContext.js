  ...
  const loadStoredAuth = async () => {
    try {
      ...
      if (storedToken && storedUser) {
        ...
        // The cached permissions can be stale (e.g. granted after the last
        // login), so sync with the server on every app start. Falls back
        // to the cached copy above if the request fails (offline, etc).
        try {
          ...
          console.error('Permission sync on startup failed:', refreshError);
        }
      }
    } catch (error) {
